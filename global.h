...
/*Level method switch*/
const bool LEVEL_SWITCH = 1;

/*Tolerance for level method*/
const double TOLLEVEL = 1e-6;
...
struct formatData
{
	...
	IloNumArray2 Amatrix; // A_t at each stage (UC: same for each stage)
	IloNumArray2 Bmatrix; // B_t at each stage
	IloNumArray2 W1matrix; // W1_t at each stage
	IloNumArray2 W2matrix; // W2_t at each stage
...
{	
	...
	IloConversion relaxVarX;
	IloConversion relaxVarY;
