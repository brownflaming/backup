...
if __name__ == "__main__":
	...
	# coefficient for y1
	y1Obj = np.zeros((numStage, dimB * 3)) # [b c z]
	...
	# coefficient for y2 [zp, zd, s1, s2]
	y2Obj = np.zeros((numStage, dimB * 2 + nrow * 2))
	penalty = 1e8 * np.ones(nrow * 2)
	for t in range(1, numStage):
		y2Obj[t][dimB*2:] = penalty 
	...
	s0 = np.zeros((dimB, dimB * 2))
	s1 = np.column_stack((np.identity(dimB), np.zeros((dimB, dimB))))
	s2 = np.column_stack((np.zeros((dimB, dimB)), np.identity(dimB) ))
	matY2 = np.array(find(np.zeros((nrow, dimB * 2 + nrow * 2))))
	W2 = [matY2.tolist()]
	matY2 = np.row_stack((np.tile(s0,(2,1)), s2, s1, -s1, np.tile(s0,(2,1)), s1, s2, s0, np.zeros((CABIN*LEG, dimB*2))))
	matY2 = np.column_stack((matY2, np.identity(nrow), -np.identity(nrow)))
	...
	if ( numStage == 2 ):
		rhs0 = np.concatenate((np.zeros(nrow-CABIN*LEG), C0))
	else:
		rhs0 = np.concatenate((np.zeros(nrow-CABIN*LEG), C0 * 2))
	rhs1 = np.concatenate((np.zeros(dimB*3), 0.5 * np.ones(dimB * 4),\
						sum(x_ub) * np.ones(dimB), np.zeros(dimB), np.ones(dimB), C0 ))
	rhs2 = np.concatenate((np.zeros(dimB*3), 0.5 * np.ones(dimB * 4),\
						sum(x_ub) * np.ones(dimB), np.zeros(dimB), np.ones(dimB), C0 * 2 ))
	...
	dcp = (182 - np.array([182, 90, 0]))/182.0
