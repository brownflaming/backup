...
if __name__ == "__main__":
	...
	# coefficient for y1
	y1Obj = np.zeros((numStage, dimB)) # [Bt+1, Bt, bt+1, Ct+1, Ct, ct+1]
	...
	A = []
	matX = np.row_stack((T, np.dot(R, T), np.zeros((dimB, dimX))))
	...
	B = []
	matZ = np.row_stack(( -T, np.zeros((nrow - dimB, dimX)) ))
	...
	matY1 = np.row_stack(( -np.identity(dimB), np.zeros((R.shape[0], dimB)), np.identity(dimB) ))
	...
	rhs = np.concatenate(( np.zeros(dimB), C0, np.zeros(dimB) ))
	for t in xrange(numStage):
		b[t] = rhs
